        ...
        while True:
            ...
            for j in batch_idxs:
                if train:
                    ...
                    video_key = key[63:81]
                    appearance_key = key[63:-9]
                else:
                    key = self.val_keys[j]
                    video_key = key[61:79]
                    appearance_key = key[61:-9]
                ...
                if len(y) == 0:
                    ...
                    if 'future_ego_motion' in self.inputs:
                        ...
                        # unshift the yaw
                        future_ego_motion[:,:,0] = future_ego_motion[:,:,0] - future_ego_motion[:,0,0]
...
#                     y = np.expand_dims(y, axis=0)
                    ...
                    if 'future_ego_motion' in self.inputs:
                        # unshift the yaw
                        tmp_future_ego_motion = np.expand_dims(np.array(data['future_ego_motion'])[:self.time_steps, [0, 3, 5]],axis=0)
                        tmp_future_ego_motion[:,:,0] = tmp_future_ego_motion[:,:,0] - tmp_future_ego_motion[:,0,0]

                        future_ego_motion = np.vstack([future_ego_motion, tmp_future_ego_motion])
            ...
            for val_file in self.val_keys:
                ...
                for name in self.inputs:
                    if name == 'depth':
                        ...
                        # unshift the yaw
                        tmp_future_ego_motion = np.expand_dims(np.array(val_trajectory['future_ego_motion'])[:time_steps, [0, 3, 5]],axis=0)
                        tmp_future_ego_motion[:,:,0] = tmp_future_ego_motion[:,:,0] - tmp_future_ego_motion[:,0,0]

                        predict_input.append(tmp_future_ego_motion)
                    else:
